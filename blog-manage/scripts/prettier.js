...
let files = [];
const tscfg = glob.sync('tsconfig.json*', { ignore: ['**/node_modules/**', 'build/**'] });
const configJsFiles = glob.sync('config/*.js*', { ignore: ['**/node_modules/**', 'build/**'] });
const jsFiles = glob.sync('src/**/*.js*', { ignore: ['**/node_modules/**', 'build/**'] });
const tsFiles = glob.sync('src/**/*.ts*', { ignore: ['**/node_modules/**', 'build/**'] });
files = files.concat(tscfg);
files = files.concat(configJsFiles);
